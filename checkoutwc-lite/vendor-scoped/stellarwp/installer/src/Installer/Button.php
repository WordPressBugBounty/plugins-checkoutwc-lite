...
class Button
{
    ...
    {
        $hook_prefix = Config::get_hook_prefix();
        $text = __('Activating...', 'checkout-wc');
        /**
    ...
    {
        $hook_prefix = Config::get_hook_prefix();
        $classes = [$this->selector_prefix, $this->selector];
        /**
    ...
    {
        $hook_prefix = Config::get_hook_prefix();
        $text = __('Installing...', 'checkout-wc');
        /**
         * Filters the installing label.
         *
         * @since 1.0.0
    ...
    public function render(string $request_action, string $button_label = null, string $redirect_url = null): void
    {
        ...
        if (empty($button_label)) {
            if ($request_action === 'activate') {
                $button_label = sprintf(__('Activate %s', 'checkout-wc'), $this->handler->get_name());
            } else {
                $button_label = sprintf(__('Install %s', 'checkout-wc'), $this->handler->get_name());
            }
        }
