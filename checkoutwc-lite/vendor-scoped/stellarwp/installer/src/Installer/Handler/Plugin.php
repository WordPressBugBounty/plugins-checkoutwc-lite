...
namespace CheckoutWC\StellarWP\Installer\Handler;

use stdClass;
use CheckoutWC\StellarWP\Installer\Button;
use CheckoutWC\StellarWP\Installer\Config;
use CheckoutWC\StellarWP\Installer\Contracts\Handler;
use CheckoutWC\StellarWP\Installer\Installer;
use CheckoutWC\StellarWP\Installer\Utils\Array_Utils;
use WP_Error;
class Plugin implements Handler
{
    ...
    {
        $hook_prefix = Config::get_hook_prefix();
        $install_url = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $this->slug), 'install-plugin_' . $this->slug);
        $message = sprintf(
            /* Translators: %1$s - opening link tag, %2$s - closing link tag. */
            __('There was an error and plugin could not be installed, %1$splease install manually%2$s.', 'checkout-wc'),
    ...
    public function get_wordpress_org_data()
    {
        ...
        $hook_prefix = Config::get_hook_prefix();
        $api_results = plugins_api('plugin_information', ['slug' => $this->slug, 'fields' => ['short_description' => \false, 'sections' => \false, 'requires' => \false, 'rating' => \false, 'ratings' => \false, 'downloaded' => \false, 'last_updated' => \false, 'added' => \false, 'tags' => \false, 'compatibility' => \false, 'homepage' => \false, 'donate_link' => \false]]);
        /**
    ...
    public function handle_request()
    {
        ...
        if (!check_ajax_referer($installer->get_nonce_name(), 'nonce', \false)) {
            $response['message'] = wpautop(__('Insecure request.', 'checkout-wc'));
            wp_send_json_error($response);
        }
        if (!current_user_can($this->get_permission())) {
            wp_send_json_error(['message' => wpautop(sprintf(__('Security Error, Need higher permissions to install %1$s.', 'checkout-wc'), $this->name))]);
        }
        $vars = ['request' => $this->get_request_var('request')];
        $success = \false;
        ...
        if (\false === $success) {
            ...
            wp_send_json_success(['message' => __('Success.', 'checkout-wc')]);
        }
    }
    /**
     * @inheritDoc
